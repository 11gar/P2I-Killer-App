...
export class GameService {
  ...
  async getAllGames() {
    ...
    const resp = await lastValueFrom(this.http.get<Game[]>(url));
    return resp;
  }
  ...
  async getGameById(id: number) {
    ...
    const resp = await lastValueFrom(
      this.http.get<Game>(url).pipe(catchError(this.handleError))
    );
    return resp;
  }
  ...
  async getPlayersFromGame(id: number) {
    ...
    const resp = await lastValueFrom(this.http.get<UserIG[]>(url));
    return resp;
  }
